...
class UnifiedLLMWrapper:
    ...
    def extract_responses_content(response) -> str:
        """Extract text from responses() API response - UPDATED VERSION"""
        # Handle incomplete responses (reasoning tokens only)
        if hasattr(response, 'status') and response.status == 'incomplete':
            # For incomplete responses, try to get any text content
            if hasattr(response, 'output') and response.output:
                ...
                if texts:
                    return "\n".join(texts)
            # If no text found in incomplete response, return a message
