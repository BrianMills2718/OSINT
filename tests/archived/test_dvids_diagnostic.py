#!/usr/bin/env python3
"""Diagnostic test for DVIDS HTTP 403 error with SIGINT query."""
import asyncio
import os
import sys
...
async def test():
    ...
    print("=" * 60)
    print("DVIDS Diagnostic Test - SIGINT Query")
    print("=" * 60)
    ...
    print(f"   Max results: {query_params.get('max_results')}")
    ...
    try:
        ...
        if result.success:
            print(f"\n✅ TEST PASSED: DVIDS working with SIGINT query")
            if result.total > 0:
