...
async def test_nspm7_investigation():
    ...
    print(f"   Max Hypotheses per Task: 4 (investigative depth)")
    print(f"   Max Hypotheses to Execute: 4")
    print(f"   Time Budget per Task: 900s (avoid per-task timeout)")
    ...
    engine.max_hypotheses_per_task = 4  # More depth for investigative runs
    engine.coverage_mode = True
    engine.max_hypotheses_to_execute = 4
    engine.max_time_per_task_seconds = 900  # Allow longer per task in test
