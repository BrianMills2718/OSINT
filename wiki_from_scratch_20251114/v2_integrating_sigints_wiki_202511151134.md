...
* Short description of what v1 does:
  ...
  * Uses a **pluggable deep-research layer** (either your existing deep_research.py system or a simple "Brave/Tavily + LLM summarizer" adapter) to gather sources.
  ...
  * Supports your style of **filter-based follow-up searches** with automation-first design.
...
* **Goals**
  ...
  * Use your existing deep_research.py system (pluggable adapter pattern allows framework options).
...
### 4.1 Deep-Research Engine (Pluggable Adapter Pattern)

* Define a very thin `DeepResearchClient` interface:
...
* Provide **two interchangeable implementations**:

  1. **DirectSearchClient**: Wraps your existing deep_research.py system (primary implementation for v1)
  2. **Optional adapter** for a library like GPT-Researcher / DeerFlow (if you want framework option later)

* The rest of your system only talks to `DeepResearchClient`, never to the specific library.

* **Builtin "direct search" path** for boolean hard-constraints:
  * Build Brave/Tavily queries directly from entities/patterns
  * Fetch pages
  * Ask LLM to summarize and list leads
  * Bypasses deep-research engine to preserve your exact boolean logic
...
2. **Use your existing deep_research.py system with clean abstraction**

   * Primary engine: Your existing deep_research.py (proven, working, automation-ready)
   * Define `DeepResearchClient` adapter interface for pluggability
   * Optional: Framework adapters (GPT-Researcher / DeerFlow) if needed later
   * Your v1 wiki **wraps** your deep research system, doesn't replace it.
...
5. **Use existing infrastructure, not build from scratch**

   * You **do not** rebuild deep research (already have deep_research.py working)
   * You **wrap** your existing system with v1 wiki features:

     * Multi-Lead organization (instead of single-run files)
     * Pattern-based expansion (instead of one-shot queries)
     * Persistent KG storage (instead of JSON dumps)
     * Automatic evidence attachment (instead of manual filing)
...
- description          # optional notes / hypothesis text
...
**Note**: A Lead is your notebook that holds all research under one umbrella. The `lead_filters_json` defines which evidence automatically belongs here (automation-first: no manual attachment needed).
...
#### 3.8 LeadRelation (cross-linking Leads)

```text
LeadRelation
- id
- from_lead_id      # FK → Lead
- to_lead_id        # FK → Lead
- relation_type     # "related_to" | "person_profile" | "background_context"
- note              # optional short text (e.g., "Hunt played role in Bay of Pigs")
```

Example use case:
- Lead A: "Bay of Pigs invasion" (main investigation)
- Lead B: "Howard Hunt career profile" (person-focused sub-investigation)
- Relation: A → "person_profile" → B

This allows cross-referencing without merging Lead contents.
...
may explode in latency and LLM attention usage if you run them aggressively

Risks

Slow feedback that kills your investigative flow (you're in the zone, system feels sluggish).

Attention overflow - too many results to process effectively, degrading quality of synthesis.

Mitigations

Make depth/breadth very visible knobs per run:

Have presets: "quick scan", "standard", "deep dive" - managing speed and cognitive load, not just cost.
...
## Appendix: Optional Context Tags (Alternative to Timelines)

**Status**: Optional enhancement - implement only if chronology/context matters during actual v1 usage

**Problem**: Timeline features (3 fields + extraction logic + UI + chronological sorting) add significant complexity and risk epistemic errors from date parsing precision.

**User Decision**: Timelines explicitly rejected for v1.

**Lightweight Alternative**: Context tags for fuzzy temporal/thematic grouping without chronological precision.

### Concept

Instead of precise timeline encoding (`start_date`, `end_date`, `time_precision`), use LLM-generated fuzzy tags during entity extraction:

```python
@dataclass
class Evidence:
    # ... existing fields (snippet_text, source_id, etc.)
    context_tags: List[str]  # ["1980s", "Cold War", "Reagan administration"]
```

### How It Works

**During Entity Extraction** (no new LLM call - piggyback on existing extraction):

```jinja2
When extracting entities from this evidence, also identify context markers:
...
Return as simple string tags for filtering/grouping.
```
...
{
  "entities": [...],
  "context_tags": ["1980s", "Cold War", "Reagan administration", "Central America"]
}
```
...
- "Show all 1980s evidence in this Lead"
- "Filter J-2 mentions tagged 'Cold War'"

**Group entities by era**:
- "Which J-2 directors appear in Cold War context?"
- "Show all programs tagged 'post-9/11'"

**No chronological sorting** - just grouping/filtering

### Benefits Over Timelines

1. **No date parsing complexity** - just fuzzy tags like "1980s", not "1985-03-15"
2. **Fuzzy by design** - avoids epistemic risk of false chronological precision
3. **Supports non-temporal context** - "Cold War", "domestic programs", "Middle East theater"
4. **LLM generates tags automatically** - no new extraction logic needed
5. **Low implementation cost** - 1-2 hours (add field + update extraction prompt)

### When to Implement

**Consider this IF**:
- You find yourself manually noting "this is Cold War stuff" during investigations
- You want to group evidence by era without precise chronology
- You need thematic/geographic context grouping

**Skip this IF**:
- Full-text search over `evidence.snippet_text` is sufficient
- Lead organization already provides enough context grouping

### Implementation Estimate

1. Add `context_tags: List[str]` to Evidence table schema (30 min)
2. Update entity extraction prompt template (30 min)
3. Add tag filtering to Evidence browser UI (30 min)

**Total**: 1-2 hours

### Decision Point

Implement in **Phase 1.5** (after 5-10 investigations) IF usage patterns show need for era/context grouping.
