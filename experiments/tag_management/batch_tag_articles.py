#!/usr/bin/env python3
"""
Batch tag articles with parallel processing.
...
def main():
    # Get all untagged articles
    input_dir = Path("/home/brian/sam_gov/klippenstein_articles_extracted")
    all_files = list(input_dir.glob("*.json"))

    # Filter out already tagged files
    untagged_files = [f for f in all_files if not f.name.endswith('_tagged.json')]

    # Select 20 articles to tag
    files_to_tag = untagged_files[:20]

    print("="*70)
    print(f"Batch Tagging Articles with gpt-5-mini")
    print("="*70)
    print(f"Total articles: {len(untagged_files)}")
    print(f"Tagging: {len(files_to_tag)}")
