#!/usr/bin/env python3
"""
Query reformulation service for deep research.

Provides LLM-based query reformulation for improved search results.
Extracted from QueryReformulationMixin to enable composition over inheritance.
...
class QueryReformulator:
    """
    Stateless service for query reformulation.

    All state is passed as parameters - no instance attributes required.
    This enables easy testing and reuse without coupling to research engine state.
    """

    # JSON schema for relevance-based reformulation
    RELEVANCE_SCHEMA = {
        "type": "object",
        "properties": {
            "query": {
    ...
    # JSON schema for simple reformulation
    SIMPLE_SCHEMA = {
        "type": "object",
        "properties": {
            "query": {
    ...
    async def reformulate_for_relevance(
        self,
        original_query: str,
        ...
        Args:
            ...
            source_performance: List of dicts with source performance data
                (name, status, results_returned, results_kept, quality_rate, error_type)
    ...
    async def reformulate_simple(
        ...
        Args:
            original_query: Current query that returned few results
            results_count: Number of results found
    ...
    async def reformulate_on_api_error(
        self,
        source_name: str,
        research_question: str,
